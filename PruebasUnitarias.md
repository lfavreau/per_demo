...
| `CASE-09` | **Nada a medias**: si falla la creación del IAP, no se persiste el caso | E14 | `mockWorkspace({ failOn: "iap" })` → se llama al rollback de carpeta (el IAP nunca llegó a crearse, no hay nada que revertir de ese lado); `provisionAndPersistCase` lanza **antes** de la transacción de Prisma, así que no queda ningún `PACase` creado. (Ya no exige Acta de Primer Encuentro — se quitó ese requisito de formalización.) |
...
| `CASE-15` | *(retirado)* Deserción ya no exige un mínimo de intentos de contacto — ese gate (`logContactAttempt`, sin ningún llamador real) se quitó por no ser parte de las métricas del convenio SENDA; Deserción exige solo el Formulario de Abandono validado, igual que Retiro Voluntario (`CASE-13`) | E28 | — |
